...
const { db, store } = require("../config/config_firebase");
...
router.post("/edit-product/:outerId/:type/:id", upload.single('image'), async (req, res) => {
  const { outerId, type, id } = req.params;
  ...
  if(productImage){
    ...
    await bucket.upload(productImage.path, {
      destination: `productImages/${fileName}`
    }).then(async (value) => {
      bucket.file(`productImages/${fileName}`).getSignedUrl({
        action: 'read',
        expires: '03-17-2025'
      }).then(async (results) => {
